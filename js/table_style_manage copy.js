function tableInfo(function_addDialog, input_dataset, input_url, input_isServerSide) {
    ////////////////////////
    //dialog List
    ////////////////////////
    var app_dialog = {
        ...
        //dataSet_history: new Array(),     //localStorage로 대체       -> 브라우저 뒤로가기 앞으로 가기 구현
    ...
    function setDialogInit(option, row_object) {
        ...
        if (option == 1) {              //add
            ...
            document.getElementById('dialog-title1-dialog_container_main').textContent = "데이터 추가";           //title 변경
            document.getElementById('dialog-title2-dialog_container_main').textContent = "각 항목의 데이터를 추가합니다.";           //title 변경
            setBackgroundColorInputbox_init('dialog_write');     //dialog의 모든 inputbox의 색상을 초기화
        } else if (option == 2) {       //edit
            if (document.getElementById('dialog-title1-dialog_container_main') == null) {
                return;
            }

            document.getElementById('dialog-title1-dialog_container_main').textContent = "데이터 수정";           //title 변경
            document.getElementById('dialog-title2-dialog_container_main').textContent = "각 항목의 데이터를 수정합니다.";           //title 변경
            setBackgroundColorInputbox_init('dialog_write');     //dialog의 모든 inputbox의 색상을 초기화
            ...
            if (getSCount < 1) {
                ...
                document.getElementById('dialog-title1-dialog_container_main').textContent = "데이터 일괄 수정";           //title 변경
                document.getElementById('dialog-title2-dialog_container_main').textContent = "각 항목에 입력된 정보만 일괄 변경되며, 입력되지 않은 필드의 정보는 기존의 데이터 그대로 유지됩니다.";           //title 변경
                setBackgroundColorInputbox_dark('dialog_write');     //dialog의 모든 inputbox 를 사용하지 못하게 하기
    ...
    //테이블 초기화 (스타일 적용)
    ////////////////////////
    var tableOption = table_style_1(app_dialog, input_url, input_isServerSide);
    ...
    if (input_isServerSide) {        
       ...
       // tableOption.bRetrieve = true,
        ...
        tableOption.ajax = ({
            ...
            //     //return inData;
            // },
            complete: function (result, aa, bb, cc) {
                ...
                //현재 리스트 해쉬데이터 생성
                var hash_data = {};
                hash_data["name"] = "#list=" + getPageNumber;
                hash_data["data"] = result.responseJSON.aaData;

                //localStorage에서 history 정보 가져오기
                var history_list = JSON.parse(localStorage.getItem("history_list"));                

                //중복 확인 후 localStorage에 저장
                var isDuplicate = false;
                if (history_list == null) {                                             //신규 저장
                    localStorage.setItem("history_list", JSON.stringify(hash_data));
                    window.location.hash = "#list=" + hash_data["name"];                  //window.location 해쉬태그 설정
                } else {                                                                  //추가 저장
                    for(i=0; i<history_list["data"].length; i++) {
                        if (history_list["name"] == hash_data["name"]) {
                            isDuplicate = true;
                            break;
                        }
                    }
                    if (isDuplicate == false) {                                             //저장
                        localStorage.setItem("history_list", JSON.stringify(hash_data));
                        window.location.hash = "#list=" + hash_data["name"];                //window.location 해쉬태그 작성
                    }
                }
            }
        });
    ...
    //테이블 초기화
    app_dialog.table = $('#table_main').DataTable(
        tableOption
    )
    ...
    $('#table_main thead').on('click', 'th', function(e){
        //첫번째 헤더를 선택하였는지 확인
        var isFind = false;
    ...
    $('#id_table_column_sequence').click(function(e) {
        // if (this.textContent == "+") {
        //     this.textContent = "-";
        //     app_dialog.table.rows(':not(.parent)').nodes().to$().find('td:first-child').trigger('click');
        // } else {
        ...
        //             //기본뷰일 경우의 설정들
        //             document.getElementById("table_main").classList.add("responsive");                                      //컬럼 자동정렬 설정
        //             document.getElementById("table_main").classList.remove("datatables_cardview");                   //카드뷰 삭제 (기본뷰 적용)
        //             node.get(0).innerText = "기본뷰";
    });
    
    //체크박스 전체 선택
    app_dialog.table.column(1).visible(true);
    $('#id_checkbox_selectAll').click(function(e) {
    ...
    $('#table_main').on('click', 'input[type="checkbox"]', function (e) {
    ...
    // //mouse over
    // $('#table_main').on('mouseenter', 'tbody tr', function (e) {

    // } );

    //mouse over
    $('#table_main').on('mouseenter', 'tbody td', function (e) {
    ...
    //Row를 선택하여 정보 수정 
    //카드뷰 인 경우 row를 선택
    $('#table_main tbody').on('click', 'tr', function(e){
        //if ($('.datatables_setview_clss').text() != "카드뷰") {
    ...
    //기본뷰인 경우 column을 선택
    $('#table_main tbody').on('click', 'td', function(e){
    ...
    // //초기화 이벤트
    // $('#table_main').on('draw.dt', function () {
    //     app_dialog.table.reduce(function (a, b) {
    //         return a + b;
    //     });
    // });

function getButtons(input_app_dialog, authStatus){
    var buttonStyle = [];
    if (authStatus) {
      buttonStyle = [
        { 
          label: 'Create',titleAttr: "새로운 항목을 생성합니다.",
          text: '추가',
          //   float: 'left',
  ...
  function setView_normalview() {
    if (document.getElementsByClassName("datatables_setview_clss")[1] != null) {
        document.getElementsByClassName("datatables_setview_clss")[1].childNodes[0].textContent = "기본뷰";
    } else {
    ...
    document.getElementById("table_main").classList.add("responsive");                               //컬럼 자동정렬 설정
    document.getElementById("table_main").classList.remove("datatables_cardview");                   //카드뷰 삭제 (기본뷰 적용)
    app_dialog.table.columns.adjust().responsive.recalc();                                           //새로 고침
    //getButton.innerText = "기본뷰";    
  }

  function setView_cardview() {        
    if (document.getElementsByClassName("datatables_setview_clss")[1] != null) {
        document.getElementsByClassName("datatables_setview_clss")[1].childNodes[0].textContent = "카드뷰";
    } else {
