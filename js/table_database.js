    ...
    loadScript("/js/table_style_manage.js", function(){
        ...
        if (isServerSide) {
            if (sessionStorage.getItem(token_yj_rfms) != null) {
                //serverside
                var url = url_json + "jsonservice/rest/databasemanage/select_databasemanageinfo_all_serverside";
            ...
            //client side
            var url = url_json + "jsonservice/rest/databasemanage/select_databasemanageinfo_all";
        ...
        window.addEventListener('message', function(event) {
            ...
            switch (event.data[0]) {
                case 1: //추가
                    ...
                    document.getElementById('dialog-title1-dialog_container_main').textContent = "데이터베이스 정보 추가";                  //title 변경
                    document.getElementById('dialog-title2-dialog_container_main').textContent = "데이터베이스 정보를 추가합니다.";          //title 변경
                    init_input();
                    ...
                    break;
                case 2: //수정
                    if (customClassInfo.db_name == null) {
                        return;
                    }

                    document.getElementById('dialog-title1-dialog_container_main').textContent = "데이터베이스 정보 수정";                  //title 변경
                    document.getElementById('dialog-title2-dialog_container_main').textContent = "데이터베이스 정보를 수정합니다.";          //title 변경
                    init_input();

                    document.getElementById('dialog_write_name').value = customClassInfo.db_name;
                    document.getElementById('dialog_write_id_sign').value = customClassInfo.db_id_sign;
                    document.getElementById('dialog_write_password_sign').value = customClassInfo.db_password_sign;
                    document.getElementById('dialog_write_dbms_type').value = customClassInfo.db_dbms_type;
                    document.getElementById('dialog_write_dbms_ip').value = customClassInfo.db_dbms_ip;
                    document.getElementById('dialog_write_dbms_name').value = customClassInfo.db_dbms_name;
                    document.getElementById('dialog_write_dbms_id').value = customClassInfo.db_dbms_id;
                    document.getElementById('dialog_write_dbms_password').value = customClassInfo.db_dbms_password;
                    document.getElementById('dialog_write_no').value = customClassInfo.no; 
                    document.getElementById('dialog_write_regist_user').value = customClassInfo.db_regist_user;
                    document.getElementById('dialog_write_regist_datetime').value = customClassInfo.db_regist_datetime;                    
                    document.getElementById('dialog_write_lastsave_datetime').value = customClassInfo.db_lastsave_datetime;
                    document.getElementById('dialog_write_etc').value = customClassInfo.db_etc;

                    //image
                    document.getElementById('print_selected_imagename').value = customClassInfo.db_image;   //이미지 경로 input
                    if ((customClassInfo.db_image == "") || (customClassInfo.db_image == null)) {
                        document.getElementById('dialog_write_image_1').src = "/images/dbms1.png";      //다이어로그 창 기본이미지
                        $('#dialog_write_image_1').attr('data-src', "/images/dbms1.png");               //이미지 상세보기 기본이미지
                    } else {
                        document.getElementById('dialog_write_image_1').src = customClassInfo.db_image;
                        $('#dialog_write_image_1').attr('data-src', customClassInfo.db_image);
                    }
                    //image round
                ...
                case 3: //일괄 수정
                    ...
                    document.getElementById('dialog-title1-dialog_container_main').textContent = "데이터베이스 정보 일괄 수정";                  //title 변경
                    ...
                    //css
                     document.getElementById('dialog_write_name').readOnly = true;                                  //데이터베이스명 수정못하게
                ...
                case 4: //삭제
                    ...
                    // document.getElementById('dialog_write_dbms_name').value = "";
        ...
        function init_input() {
            ...
            document.getElementById('dialog_write_id_sign').value = "";
            document.getElementById('dialog_write_password_sign').value = "";
            document.getElementById('dialog_write_dbms_type').value = "Oracle";
            document.getElementById('dialog_write_dbms_ip').value = "";
            document.getElementById('dialog_write_dbms_name').value = "";
            document.getElementById('dialog_write_dbms_id').value = "";
            document.getElementById('dialog_write_dbms_password').value = "";
            ...
            //image
            document.getElementById('dialog_write_image_1').src = "/images/dbms1.png";
            $('#dialog_write_image_1').attr('data-src', "/images/dbms1.png");    //이미지 상세보기  기본이미지
            if (document.getElementById("dialog_write_image_1").classList.contains("round_image") == false) {
                document.getElementById("dialog_write_image_1").classList.add("round_image");   
            }
...
function table_style_1(input_app_dialog, input_url, input_isServerSide){    
    var returnObject = {
        ...
        initComplete: function () {       //초기화가 완료된 후 이벤트 메시지
            ...
            $('.useid_datatables_edit').css("display", "none");     //데이터베이스정보에서 일괄 수정이 필요없으므로 보이지 않게 함
        },
        ...
        columns: [{     //순차적 인덱스 (client에서만 사용)
                ...
                data: 'db_dbms_password',
                title: '데이터베이스 비밀번호',
                className: 'dt-body-center cardview_topright cardview_tr_setheight',      //cardview_startcontent: 카드뷰일 경우 이 필드부터 내용 시작 (두번째 줄 출력이며, 첫번째 줄은 시퀀스 번호와 체크박스), datatables_maxColumn: 최대 컬럼 길이 설정
                orderable: true,
                searchable: true,
            },{
                data: 'db_regist_datetime',
                title: '등록일',
