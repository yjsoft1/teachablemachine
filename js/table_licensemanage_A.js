...
$(document).ready(function() {
    loadScript("/js/table_style_manage_A.js", function(){
        ...
        // //테스트
        // app_dialog_A = tableInfo_A('dialog_licensemanage_A', JSON_POST_TEST());        //리스트뷰 생성
        ...
        if (isServerSide) {
            if (sessionStorage.getItem(token_yj_rfms) != null) {
                //serverside
                var url = url_json + "jsonservice/rest/licensemanage/select_licensemanageinfo_all_serverside";
                //json 데이터 수신 후 테이블 생성하기
                app_dialog_A = tableInfo_A('dialog_licensemanage_A', null, url, isServerSide);           //리스트뷰 생성            
            ...
            //client side
            var url = url_json + "jsonservice/rest/licensemanage/select_licensemanageinfo_all";
            JSON_POST(url, null, true, sessionStorage.getItem(token_yj_rfms), function (result) {
                setTimeout(function() {
                    //json 데이터 수신 후 테이블 생성하기
                    if (result[0] == "ok") {
                        app_dialog_A = tableInfo_A('dialog_licensemanage_A', result[1], url, isServerSide);      //리스트뷰 생성
                    } else {                                
        ...
        window.addEventListener('message', function(event) {
            ...
            switch (event.data[0]) {
                case 1: //추가
                    document.getElementById('dialog-title1-dialog_licensemanage_A').textContent = "라이선스 정보 추가";                       //title 변경
                    document.getElementById('dialog-title2-dialog_licensemanage_A').textContent = "라이선스 정보를 추가합니다.";          //title 변경                    
                    init_input();
                    ...
                    break;
                case 2: //수정
                    document.getElementById('dialog-title1-dialog_licensemanage_A').textContent = "라이선스 정보 수정";                  //title 변경
                    document.getElementById('dialog-title2-dialog_licensemanage_A').textContent = "라이선스 정보를 수정합니다.";          //title 변경
                    init_input();

                    document.getElementById('dialog_write_allow_A').value = customClassInfo.li_allow;
                    document.getElementById('dialog_write_name_A').value = customClassInfo.li_name;
                    document.getElementById('dialog_write_type_A').value = customClassInfo.li_type;
                    document.getElementById('dialog_write_mac_A').value =  customClassInfo.li_mac;
                    document.getElementById('dialog_write_requester_A').value = customClassInfo.li_requester;
                    document.getElementById('dialog_write_licenser_A').value = customClassInfo.li_licenser;
                    document.getElementById('dialog_write_site_A').value = customClassInfo.li_site;
                    document.getElementById('dialog_write_dept_A').value =  customClassInfo.li_dept;
                    document.getElementById('dialog_write_no_A').value = customClassInfo.no; 
                    document.getElementById('dialog_write_regist_datetime_A').value = customClassInfo.li_regist_datetime;
                    document.getElementById('dialog_write_lastsave_datetime_A').value = customClassInfo.li_lastsave_datetime;
                    document.getElementById('dialog_write_etc_A').value = customClassInfo.li_etc;

                    //image
                    document.getElementById('print_selected_imagename_A').value = customClassInfo.li_image;   //이미지 경로 input
                    if ((customClassInfo.li_image == "") || (customClassInfo.li_image == null)) {
                        ...
                        document.getElementById('dialog_write_image_1_A').src = customClassInfo.li_image;
                        $('#dialog_write_image_1_A').attr('data-src', customClassInfo.li_image);
                    }
                    //image round
                ...
                case 3: //일괄 수정
                    ...
                    document.getElementById('dialog-title1-dialog_licensemanage_A').textContent = "라이선스 정보 일괄 수정";                  //title 변경
                    document.getElementById('dialog-title2-dialog_licensemanage_A').textContent = "선택하신 " + getSCount + "개의 데이터에 아래의 각 항목에 입력된 정보만 일괄 변경되며, 입력되지 않은 필드의 정보는 기존의 데이터 그대로 유지됩니다.";           //title 변경
                    init_input();
        ...
        function init_input() {
            document.getElementById('dialog_write_allow_A').value = "";
            document.getElementById('dialog_write_name_A').value = "";
            document.getElementById('dialog_write_type_A').value = "";
            document.getElementById('dialog_write_mac_A').value = "";
            document.getElementById('dialog_write_requester_A').value = "";
            document.getElementById('dialog_write_licenser_A').value = "";
            document.getElementById('dialog_write_site_A').value = "";
...
function table_style_A(input_app_dialog, input_url, input_isServerSide){    
    var returnObject = {
        ...
        initComplete: function () {       //초기화가 완료된 후 이벤트 메시지
            ...
            $('.useid_datatables_edit').css("display", "none");     //라이선스정보에서 일괄 수정이 필요없으므로 보이지 않게 함
        ...
        columns: [{     //순차적 인덱스 (client에서만 사용)
                ...
                //tableid_table_licensemanage_A => 현재 테이블을 구분짓는 고유 이름(팝업 찾기에서 고유이름 필요)
                className: 'dt-body-center dataTables_column_sequence_id class_table_column_sequence_wrap cardview_sequence cardview_tr_leftzero tableid_table_licensemanage_A',
