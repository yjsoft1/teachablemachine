...
$(document).ready(function() {
    loadScript("/js/table_style_pop.js", function(){
        ...
        // //테스트
        // app_dialog_pop = tableInfo_pop('dialog_location_pop', JSON_POST_TEST());        //리스트뷰 생성
        ...
        if (isServerSide) {
            if (sessionStorage.getItem(token_yj_rfms) != null) {
                //serverside
                var url = url_json + "jsonservice/rest/location/select_locationinfo_all_serverside";
                //json 데이터 수신 후 테이블 생성하기
                app_dialog_pop = tableInfo_pop('dialog_location_pop', null, url, isServerSide);           //리스트뷰 생성            
            ...
            //client side
            var url = url_json + "jsonservice/rest/location/select_locationinfo_all";
            JSON_POST(url, null, true, sessionStorage.getItem(token_yj_rfms), function (result) {
                setTimeout(function() {
                    //json 데이터 수신 후 테이블 생성하기
                    if (result[0] == "ok") {
                        app_dialog_pop = tableInfo_pop('dialog_location_pop', result[1], url, isServerSide);      //리스트뷰 생성
                    } else {                                
        ...
        window.addEventListener('message', function(event) {
            ...
            switch (event.data[0]) {
                case 1: //추가
                    ...
                    document.getElementById('dialog-title1-dialog_container_main_pop').textContent = "위치 정보 추가";                       //title 변경
                    document.getElementById('dialog-title2-dialog_container_main_pop').textContent = "위치 정보를 추가합니다.";          //title 변경
                    init_input();
                    
                    //mustimage
                    document.getElementById('dialog_write_locationinfo_name_pop').classList.add('mustimage');
                    ...
                    break;
                case 2: //수정
                    if (customClassInfo.lo_name == null) {
                        return;
                    }    

                    document.getElementById('dialog-title1-dialog_container_main_pop').textContent = "위치 정보 수정";                  //title 변경
                    document.getElementById('dialog-title2-dialog_container_main_pop').textContent = "위치 정보를 수정합니다.";          //title 변경
                    init_input();

                    document.getElementById('dialog_write_locationinfo_name_pop').value = customClassInfo.lo_name;
                    document.getElementById('dialog_write_no_pop').value = customClassInfo.no; 
                    document.getElementById('dialog_write_regist_datetime_pop').value = customClassInfo.lo_regist_datetime;
                    document.getElementById('dialog_write_lastsave_datetime_pop').value = customClassInfo.lo_lastsave_datetime;
                ...
                case 3: //일괄 수정
                    ...
                    document.getElementById('dialog-title1-dialog_container_main_pop').textContent = "위치 정보 일괄 수정";                  //title 변경
        ...
        function init_input() {
            document.getElementById('dialog_write_locationinfo_name_pop').value = "";
...
function table_style_pop(input_app_dialog, input_url, input_isServerSide){    
    var returnObject = {
        ...
        initComplete: function () {       //초기화가 완료된 후 이벤트 메시지
            ...
            $('.useid_datatables_edit').css("display", "none");     //위치정보에서 일괄 수정이 필요없으므로 보이지 않게 함
        },
        ...
        columns: [{     //순차적 인덱스 (client에서만 사용)
                ...
                //tableid_table_location_pop => 현재 테이블을 구분짓는 고유 이름(팝업 찾기에서 고유이름 필요)
                className: 'dt-body-center dataTables_column_sequence_id class_table_column_sequence_wrap cardview_sequence cardview_tr_leftzero tableid_table_location_pop',
