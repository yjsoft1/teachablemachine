...
$(document).ready(function() {
    loadScript("/js/table_style_manage.js", function(){
        ...
        if (isServerSide) {
            ...
            //     //serverside
            //     var url = url_json + "jsonservice/rest/user/select_deviceinfo_all_serverside";
            ...
            //client side
            var url = url_json + "jsonservice/rest/user/select_deviceinfo_all";
        ...
        window.addEventListener('message', function(event) {
            ...
            switch (event.data[0]) {
                ...
                case 2: //수정
                    ...
                    document.getElementById('dialog_write_user_id').value = customClassInfo.cf_id;
                    document.getElementById('dialog_write_password').value = "";
                    document.getElementById('dialog_write_name').value = customClassInfo.cf_name;
                    document.getElementById('dialog_write_email').value = customClassInfo.cf_email;
                    document.getElementById('dialog_write_mobile').value = customClassInfo.cf_mobile;      
                    document.getElementById('dialog_write_no').value = customClassInfo.no; 
                    document.getElementById('dialog_write_regist_datetime').value = customClassInfo.cf_regist_datetime;
                    document.getElementById('dialog_write_lastsave_datetime').value = customClassInfo.cf_lastsave_datetime;
                    document.getElementById('dialog_write_license_date').value = customClassInfo.cf_license_date;
                    
                    // document.getElementById('dialog_write_auth').options.value = customClassInfo.cf_auth;
                    // document.getElementById('dialog_write_dept').value = customClassInfo.cf_dept;   
                    
                    //셀렉트 박스
                    var dwa = document.getElementById('dialog_write_auth');
                    for (var k=0; k<dwa.length; k++) {
                        if (dwa.options[k].text == customClassInfo.cf_auth) {
                            dwa.selectedIndex = k;
                        }
                    }
                    ...
                    for (var k=0; k<dwd.length; k++) {
                        if (dwd.options[k].text == customClassInfo.cf_dept) {
                            dwd.selectedIndex = k;
                        }
                    }

                    var dwd = document.getElementById('dialog_write_position');
                    for (var k=0; k<dwd.length; k++) {
                        if (dwd.options[k].text == customClassInfo.cf_position) {
                            dwd.selectedIndex = k;
                        }
                    }

                    var dwd = document.getElementById('dialog_write_statususer');
                    for (var k=0; k<dwd.length; k++) {
                        if (dwd.options[k].text == customClassInfo.cf_statususer) {
                            dwd.selectedIndex = k;
                        }
                    }


                    document.getElementById('dialog_write_x').value = customClassInfo.cf_x;
                    document.getElementById('dialog_write_y').value = customClassInfo.cf_y;
                    document.getElementById('dialog_write_code_device').value = customClassInfo.cf_code_device;
                    //image
...
function table_style_1(input_app_dialog, input_url, input_isServerSide){
    var returnObject = {
        ...
        columns: [{     //순차적 인덱스 (client에서만 사용)
                ...
                className: 'dt-body-center dataTables_column_sequence_id class_table_column_sequence_wrap cardview_sequence cardview_tr_leftzero',
