...
var app_dialog;
var tableDetailList_useBackButton;
    ...
    loadScript("/js/table_style_manage.js", function(){
        ...
        jQuery(window).on('hashchange', function (e) {
            ...
            if (hash_tag[0] == "#view") {
                ...
                //리스트가 없는 경우 초기화
                if (tableDetailList_useBackButton == null) {
                    tableDetailList_useBackButton = app_dialog.dataSet;
                } else {
                    //리스트에상세정보 저장하기                
                    for (var k = 0; k < app_dialog.dataSet.length; k++) {
                        //중복 데이터인지 확인
                        var isDuplicate = false;
                        for (var i=0; i<tableDetailList_useBackButton.length; i++) {
                            if (app_dialog.dataSet[k].no == tableDetailList_useBackButton[i].no) {
                                isDuplicate = true;
                                break;
                            }
                        }
                        ...
                        if (isDuplicate == false) {
                            tableDetailList_useBackButton.push(app_dialog.dataSet[k]);
                        }
                    }
                }

                //디테일 정보 리스트에 저장된 내용을 해쉬태그의 정보와 동일한지 확인 후 디테일 정보 저장
                for (var k = 0; k < tableDetailList_useBackButton.length; k++) {
                    if (tableDetailList_useBackButton[k].no == hash_tag[1]) {
                        customClassInfo = tableDetailList_useBackButton[k];
                        break;
                    }
                }
                ...
                // //디테일 정보 리스트에 저장된 내용을 해쉬태그의 정보와 동일한지 확인 후 디테일 정보 저장
                // for (var k = 0; k < app_dialog.dataSet_history.length; k++) {
                //     if (app_dialog.dataSet_history[k]["name"] == hash_tag[1]) {
                //         app_dialog.dataSet = app_dialog.dataSet_history[k];
