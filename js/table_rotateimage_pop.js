...
$(document).ready(function() {
    loadScript("/js/table_style_pop.js", function(){
        ...
        // //테스트
        // app_dialog_pop = tableInfo_pop('dialog_rotateimage_pop', JSON_POST_TEST());        //리스트뷰 생성
        ...
        if (isLocalSide) {
                ...
                store.openCursor().onsuccess = function(event) {
                    ...
                    if (cursor) {
                        var rotateimageinfo = {
                            ...
                            rt_regist_datetime: cursor.value.rt_regist_datetime,
                            rt_lastsave_datetime: cursor.value.rt_lastsave_datetime,
                            rt_image: cursor.value.rt_image,
                            rt_etc: cursor.value.rt_etc,
                        };
                        list_rotateimageinfo.push(rotateimageinfo);

                        cursor.continue();
                    } else {
                        app_dialog_pop = tableInfo_pop('dialog_rotateimage_pop', list_rotateimageinfo, url, isServerSide);      //리스트뷰 생성
            ...
            if (isServerSide) {
                if (sessionStorage.getItem(token_yj_rfms) != null) {
                    //serverside
                    var url = url_json + "jsonservice/rest/rotateimage/select_rotateimageinfo_all_serverside";
                    //json 데이터 수신 후 테이블 생성하기
                    app_dialog_pop = tableInfo_pop('dialog_rotateimage_pop', null, url, isServerSide);           //리스트뷰 생성            
                    setPaginateCount_pop(5);                                                                //paginate size
                ...
                //client side
                var url = url_json + "jsonservice/rest/rotateimage/select_rotateimageinfo_all";
                JSON_POST(url, null, true, sessionStorage.getItem(token_yj_rfms), function (result) {
                    setTimeout(function() {
                        //json 데이터 수신 후 테이블 생성하기
                        if (result[0] == "ok") {
                            app_dialog_pop = tableInfo_pop('dialog_rotateimage_pop', result[1], url, isServerSide);      //리스트뷰 생성
                        } else {                                
        ...
        window.addEventListener('message', function(event) {
            ...
            switch (event.data[0]) {
                case 1: //추가
                    ...
                    document.getElementById('dialog-title1-dialog_container_main_pop').textContent = "이미지컨텐츠 정보 추가";                       //title 변경
                    document.getElementById('dialog-title2-dialog_container_main_pop').textContent = "이미지컨텐츠 정보를 추가합니다.";          //title 변경
                    init_input();

                    //mustimage
                    document.getElementById('dialog_write_rotateimageinfo_name_pop').classList.add('mustimage');
                    ...
                    break;
                case 2: //수정
                    if (customClassInfo.rt_name == null) {
                        return;
                    }    

                    document.getElementById('dialog-title1-dialog_container_main_pop').textContent = "이미지컨텐츠 정보 수정";                  //title 변경
                    document.getElementById('dialog-title2-dialog_container_main_pop').textContent = "이미지컨텐츠 정보를 수정합니다.";          //title 변경
                    init_input();

                    document.getElementById('dialog_write_rotateimageinfo_name_pop').value = customClassInfo.rt_name;
                    document.getElementById('dialog_write_no_pop').value = customClassInfo.no; 
                    document.getElementById('dialog_write_regist_datetime_pop').value = customClassInfo.rt_regist_datetime;
                    document.getElementById('dialog_write_lastsave_datetime_pop').value = customClassInfo.rt_lastsave_datetime;

                    var dwt = document.getElementById('dialog_write_type');
                    for (var k=0; k<dwt.length; k++) {
                        if (dwt.options[k].text == customClassInfo.rt_content_type_userstring) {
                            dwt.selectedIndex = k;
                        }
                    }

                    //image
                    if (customClassInfo.rt_image != null) {
                        //document.getElementById('print_selected_imagename_pop').value = customClassInfo.rt_image;   //이미지 경로 input
                    }
                    
                    if ((customClassInfo.rt_image == "") || (customClassInfo.rt_image == null)) {
                        ...
                        document.getElementById('dialog_write_image_1_pop').src = customClassInfo.rt_image;
                        $('#dialog_write_image_1_pop').attr('data-src', customClassInfo.rt_image);
                    }
                    //image round
                ...
                case 3: //일괄 수정
                    ...
                    document.getElementById('dialog-title1-dialog_container_main_pop').textContent = "이미지컨텐츠 정보 일괄 수정";                  //title 변경
                    init_input();
        ...
        function init_input() {
            document.getElementById('dialog_write_rotateimageinfo_name_pop').value = "";
            ...
            document.getElementById('print_selected_imagename_pop').value = "";
...
function table_style_pop(input_app_dialog, input_url, input_isServerSide){    
    var returnObject = {
        ...
        initComplete: function () {       //초기화가 완료된 후 이벤트 메시지
            ...
            $('.useid_datatables_edit').css("display", "none");     //이미지컨텐츠정보에서 일괄 수정이 필요없으므로 보이지 않게 함
        },
        ...
        columns: [{     //순차적 인덱스 (client에서만 사용)
                ...
                //tableid_table_rotateimage_pop => 현재 테이블을 구분짓는 고유 이름(팝업 찾기에서 고유이름 필요)
                className: 'dt-body-center dataTables_column_sequence_id class_table_column_sequence_wrap cardview_sequence cardview_tr_leftzero tableid_table_rotateimage_pop',
                ...
                data: 'rt_content_type_userstring',
                title: '분류',
                className: 'dt-body-center cardview_startcontent cardview_tr_leftzero cardview_tr_setheight datatables_maxColumn',      //cardview_startcontent: 카드뷰일 경우 이 필드부터 내용 시작 (두번째 줄 출력이며, 첫번째 줄은 시퀀스 번호와 체크박스), datatables_maxColumn: 최대 컬럼 길이 설정
                orderable: true,
                searchable: true,
