...
$(document).ready(function() {
    loadScript("/js/table_style_pop.js", function(){
        ...
        // //테스트
        // app_dialog_pop = tableInfo_pop('dialog_asset_pop', JSON_POST_TEST());        //리스트뷰 생성
        ...
        if (isServerSide) {
            if (sessionStorage.getItem(token_yj_rfms) != null) {
                //serverside
                var url = url_json + "jsonservice/rest/asset/select_assetinfo_all_serverside";
                //json 데이터 수신 후 테이블 생성하기
                app_dialog_pop = tableInfo_pop('dialog_asset_pop', null, url, isServerSide);           //리스트뷰 생성            
            ...
            //client side
            var url = url_json + "jsonservice/rest/asset/select_assetinfo_all";
            JSON_POST(url, null, true, sessionStorage.getItem(token_yj_rfms), function (result) {
                setTimeout(function() {
                    //json 데이터 수신 후 테이블 생성하기
                    if (result[0] == "ok") {
                        app_dialog_pop = tableInfo_pop('dialog_asset_pop', result[1], url, isServerSide);      //리스트뷰 생성
                    } else {                                
        ...
        window.addEventListener('message', function(event) {
            ...
            switch (event.data[0]) {
                ...
                case 2: //수정
                    ...
                    document.getElementById('dialog_write_assetinfo_name_pop').value = customClassInfo.at_name;
        ...
        function init_input() {
            document.getElementById('dialog_write_assetinfo_name_pop').value = "";
            ...
            //mustimage
            document.getElementById('dialog_write_assetinfo_name_pop').classList.add('mustimage');
        }
    });
});
...
function table_style_pop(input_app_dialog, input_url, input_isServerSide){    
    var returnObject = {
        ...
        columns: [{     //순차적 인덱스 (client에서만 사용)
                ...
                //tableid_table_asset_pop => 현재 테이블을 구분짓는 고유 이름(팝업 찾기에서 고유이름 필요)
                className: 'dt-body-center dataTables_column_sequence_id class_table_column_sequence_wrap cardview_sequence cardview_tr_leftzero tableid_table_asset_pop',
