function tableInfo_pop(function_addDialog, input_dataset, input_url, input_isServerSide) {
    ////////////////////////
    //dialog List
    ////////////////////////
    var app_dialog = {
    ...
    function setDialogInit(option, row_object) {
        ...
        try{
            document.getElementsByClassName("modal-window")[0].scrollTop = 0; // where x is some integer
            //document.getElementsByClassName("modal-window")[0].scrollTo(0, 0);
        } catch (e) {
        }
        ...
        if (option == 1) {              //add
            ...
            document.getElementById('dialog-title1-dialog_container_main_pop').textContent = "데이터 추가";           //title 변경
            document.getElementById('dialog-title2-dialog_container_main_pop').textContent = "각 항목의 데이터를 추가합니다.";           //title 변경
            setBackgroundColorInputbox_init('dialog_write_pop');     //dialog의 모든 inputbox의 색상을 초기화
        } else if (option == 2) {       //edit
            if (document.getElementById('dialog-title1-dialog_container_main_pop') == null) {
                return;
            }

            document.getElementById('dialog-title1-dialog_container_main_pop').textContent = "데이터 수정";           //title 변경
            document.getElementById('dialog-title2-dialog_container_main_pop').textContent = "각 항목의 데이터를 수정합니다.";           //title 변경
            setBackgroundColorInputbox_init('dialog_write_pop');     //dialog의 모든 inputbox의 색상을 초기화
            ...
            if (getSCount < 1) {
                ...
                document.getElementById('dialog-title1-dialog_container_main_pop').textContent = "데이터 일괄 수정";           //title 변경
                document.getElementById('dialog-title2-dialog_container_main_pop').textContent = "각 항목에 입력된 정보만 일괄 변경되며, 입력되지 않은 필드의 정보는 기존의 데이터 그대로 유지됩니다.";           //title 변경
                setBackgroundColorInputbox_dark('dialog_write_pop');     //dialog의 모든 inputbox 를 사용하지 못하게 하기
    ...
    //테이블 초기화 (스타일 적용)
    ////////////////////////
    var tableOption = table_style_pop(app_dialog, input_url, input_isServerSide);
    ...
    //테이블 초기화
    app_dialog.table = $('#table_main_pop').DataTable(
        tableOption
    )    
    ...
    $('#table_main_pop thead').on('click', 'th', function(e){
        //첫번째 헤더를 선택하였는지 확인
        var isFind = false;
    ...
    $('#id_table_column_sequence_pop').click(function(e) {
        // if (this.textContent == "+") {
        //     this.textContent = "-";
        //     app_dialog.table.rows(':not(.parent)').nodes().to$().find('td:first-child').trigger('click');
        // } else {
        ...
        // document.getElementById("table_main_pop").classList.remove("responsive");                                   //컬럼 자동정렬 해제
        //             document.getElementById("table_main_pop").classList.add("datatables_cardview");                      //카드뷰 적용
        ...
        //             //기본뷰일 경우의 설정들
        //             document.getElementById("table_main_pop").classList.add("responsive");                                      //컬럼 자동정렬 설정
        //             document.getElementById("table_main_pop").classList.remove("datatables_cardview");                   //카드뷰 삭제 (기본뷰 적용)
        //             node.get(0).innerText = "기본뷰";
    });
    
    //체크박스 전체 선택
    $('#id_checkbox_selectAll_pop').click(function(e) {
    ...
    $('#table_main_pop').on('click', 'input[type="checkbox"]', function (e) {
    ...
    // //mouse over
    // $('#table_main_pop').on('mouseenter', 'tbody tr', function (e) {

    // } );

    //mouse over
    $('#table_main_pop').on('mouseenter', 'tbody td', function (e) {
    ...
    //Row를 선택하여 정보 수정 
    //카드뷰 인 경우 row를 선택
    $('#table_main_pop tbody').on('click', 'tr', function(e){
        if ($('.datatables_setview_clss_pop').text() != "카드뷰") {
            return;
        }
    ...
    //기본뷰인 경우 column을 선택
    $('#table_main_pop tbody').on('click', 'td', function(e){
        if ($('.datatables_setview_clss_pop').text() != "기본뷰") {
            return;
        }
    ...
    // //초기화 이벤트
    // $('#table_main_pop').on('draw.dt', function () {
    //     app_dialog.table.reduce(function (a, b) {
    //         return a + b;
    //     });
    // });

function getButtons_pop(input_app_dialog, authStatus){
    var buttonStyle = [];
    if (authStatus) {
      buttonStyle = [
        { 
        ...
        },
        {
          ...
          action: function ( e, dt, node, config ) {
            ...
            // $.getJSON("http://localhost:8080/jsonservice/jsonService/rfms/getUserInfo", function(data){
            //   document.write(data);
            // });
          },
        ...
        },        
        {
          ...
          className: 'datatables_button_image_excel'
          // select: true,
          // dom: 'Bfrtip',
        },
        {
            ...
            className: 'btn btn-primary datatables_setview_clss_pop datatables_button_image_view'
        },
        // {
        //   extend: 'pdfHtml5',
...
function setView_normalview_pop() {
    document.getElementsByClassName("datatables_setview_clss_pop")[0].childNodes[0].textContent = "기본뷰";

    //기본뷰일 경우의 설정들
    document.getElementById("table_main_pop").classList.add("responsive");                               //컬럼 자동정렬 설정
    document.getElementById("table_main_pop").classList.remove("datatables_cardview");                   //카드뷰 삭제 (기본뷰 적용)
    app_dialog_pop.table.columns.adjust().responsive.recalc();                                           //새로 고침
    //getButton.innerText = "기본뷰";    
}

function setView_cardview_pop() {
    try {
        document.getElementsByClassName("datatables_setview_clss_pop")[0].childNodes[0].textContent = "카드뷰";
