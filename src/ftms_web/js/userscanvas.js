...
var get_selected_no;            //사용자가 선택한 유저 고유 번호
...
$(document).ready(function(e) {    
    ...
    var template6 = document.getElementsByClassName("user_info_template_6")[0];     //get user_info_template_6
    var template_range1 = document.getElementsByClassName("user_info_template_range1")[0];     //get user_info_template_6
        ...
        store.openCursor().onsuccess = function(event) {
            ...
            if (cursor) {
                var memberinfo = {
                    ...
                    mb_work_state: cursor.value.mb_work_state,
                ...
                //사용자 그리기
                create_Element(list_users, canvas_main, template1, template1_1, template2, template3, template4, template5, template6, template_range1);
            }
         };
    });

    // //사용자 데이터 수신
    // var url = url_json + "jsonservice/rfms/select_userinfo_all";
...
//사용자 그리기
function create_Element(list_users, canvas_main, template1, template1_1, template2, template3, template4, template5, template6, template_range1) {
    ...
    for (var i=0; i<list_users.length; i++) {
        ...
        if (list_users[i].mb_type != null) {
            ...
            } else if (list_users[i].mb_type == "type1_1") {
            ...
            } else if (list_users[i].mb_type == "type6") {
                getElement = template6.cloneNode(true);      //create element
            } else if (list_users[i].mb_type == "type_range1") {
        ...
        //근무 상태 적용
        if (list_users[i].mb_work_state == "근무중") {
            setCanvasUserState_background_color(true, list_users[i].no, list_users[i].mb_name, list_users[i].mb_type);
        } else if (list_users[i].mb_work_state == "부재중") {
            setCanvasUserState_background_color(false, list_users[i].no, list_users[i].mb_name, list_users[i].mb_type);
        } else {
            setCanvasUserState_background_color(true, list_users[i].no, list_users[i].mb_name, list_users[i].mb_type);
        }
    }
...
function create_Element_one(memberinfo, canvas_main, template1, template1_1, template2, template3, template4, template5, template6, template_range1) {
    var getElement = template1.cloneNode(true);      //create element

    //create template
    if (memberinfo.mb_type != null) {
        if (memberinfo.mb_type == "type1") {
            getElement = template1.cloneNode(true);      //create element
        } else if (memberinfo.mb_type == "type1_1") {
            getElement = template1_1.cloneNode(true);      //create element
        } else if (memberinfo.mb_type == "type2") {
            getElement = template2.cloneNode(true);      //create element
        } else if (memberinfo.mb_type == "type3") {
            getElement = template3.cloneNode(true);      //create element
        } else if (memberinfo.mb_type == "type4") {
            getElement = template4.cloneNode(true);      //create element
        } else if (memberinfo.mb_type == "type5") {
            getElement = template5.cloneNode(true);      //create element
        } else if (memberinfo.mb_type == "type6") {
            getElement = template6.cloneNode(true);      //create element
        } else if (memberinfo.mb_type == "type_range1") {
            getElement = template_range1.cloneNode(true);      //create element
        }
    } else {
        getElement = template1.cloneNode(true);      //create element
    ...
    canvas_main.appendChild(getElement);            //add gui  
    dragElement(getElement);                        //set drag
    setElement_init(getElement, memberinfo);     //set data

    //근무 상태 적용
    if (memberinfo.mb_work_state == "근무중") {
        setCanvasUserState_background_color(true, memberinfo.no, memberinfo.mb_name, memberinfo.mb_type);
    } else if (memberinfo.mb_work_state == "부재중") {
        setCanvasUserState_background_color(false, memberinfo.no, memberinfo.mb_name, memberinfo.mb_type);
    } else {
        setCanvasUserState_background_color(true, memberinfo.no, memberinfo.mb_name, memberinfo.mb_type);
    }
}

function setCanvasUserState_background_color(isWork, mb_id, mb_name, mb_type) {
    var canvas_main = document.getElementById('userscanvas_main');
    var get_elements = canvas_main.getElementsByClassName("user_info_canvas_label_text_no")

    for (var i = 0; i < get_elements.length; i++) {
        if (parseInt(get_elements[i].textContent) == parseInt(mb_id)) {
            ...
            //범례와 일반 직원템플릿을 구분하여 Border 설정
            if ((mb_name == "근무중") || (mb_name == "부재중")) {       //범례일 경우
                userImageBorder_element.style.backgroundColor = "#ffffff";
            } else {                                                                                   //범례가 아닌 일반 직원일 경우
                if (isWork == true) {
                    if (mb_type != null)
                    {
                        if (mb_type == "type_range1")
                        {
                            if (!userImageBorder_element.classList.contains("noselect"))
                                userImageBorder_element.classList.add("noselect");
                        }
                        else {
...
function dragElement(element) {
    ...
    function dragMouseDown(e) {
        ...
        if (sessionStorage.getItem("canvas_menu") == "isShowUserInfo") {
            ...
            if (isEnableElement == true) {
                if (e.currentTarget.className == "user_info_template_range1")
                {
                    return;
                }
    ...
    function closeDragElement(e) {
        ...
        get_selected_no = e.target.parentElement.parentElement.parentElement                  //no 가져오기
...
function setElement_init(input_element, users) {
    ...
    //Content 설정
    if (childElement.getElementsByClassName("position_name")[0] != null)
    {
        if (users.mb_type == "type_range1")
        {
            childElement.getElementsByClassName("position_name")[0].textContent = users.mb_name;
            childElement.getElementsByClassName("stretch_wrap")[0].style.width = users.mb_task_1;
            childElement.getElementsByClassName("stretch_wrap")[0].style.height = users.mb_task_2;
            childElement.getElementsByClassName("user_info_template_content_1")[0].style.backgroundColor = users.mb_task_3;
        }
        else
            childElement.getElementsByClassName("position_name")[0].textContent = users.mb_position;
    }
...
function send_xyinfo(input_userinfo) {
    ...
    // //JSON
    // var url = url_json + "jsonservice/rfms/update_xy";
...
function userinfo_show(visible_oneButton, title, content, result, visible_spinner, e) {
    ...
    //근무 상태 적용
    if (get_selected_userinfo.mb_work_state == "근무중") {
        $("#dialog_write_user_id_wrap .stretch_left_100percent").css("border-color", "#f1f1f1");        //근무 상태 초기화
    } else if (get_selected_userinfo.mb_work_state == "부재중") {
        $("#dialog_write_user_id_wrap .stretch_left_100percent").css("border-color", "#ff6600");        //근무 상태 초기화
    } else {
        $("#dialog_write_user_id_wrap .stretch_left_100percent").css("border-color", "#f1f1f1");        //근무 상태 초기화
    }

    //범례정보일 경우 되돌아가기
    if (get_selected_userinfo.mb_name == "부재중")
        return;

    if (get_selected_userinfo.mb_name == "근무중")
        return;
